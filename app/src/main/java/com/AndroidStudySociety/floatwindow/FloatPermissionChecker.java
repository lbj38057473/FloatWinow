...
public class FloatPermissionChecker {
    private static final String TAG = "FloatPermissionChecker";
    ...
    private static boolean checkOps(Context context) {
        try {
            ...
            arrayOfObject1[2] = context.getApplicationContext().getApplicationContext().getPackageName();
    ...
    public static void jumpToPermissonPage(Context context){
        tryJumpToPermissonPage(context);
    }
    ...
    private static void applyVivoPermission(Context context) {
        try {
            ...
            if (isIntentAvailable(appIntent,context)){
